(function (window, undefined) {
    ...
    var ifeng = {},
	rfocusable = /^(?:button|input|object|select|textarea)gf/i;
    var isPhotoImageLoaded = false;
    var staSign = true;
    Function.prototype.bind = function () {
        if (!arguments.length) return this;
        var _ = this,
            gf = Array.prototype.slice.call(arguments),
            A = gf.shift();
        return function () {
            return _.apply(A, gf.concat(Array.prototype.slice.call(arguments)))
        }
    };
    var removeListener;
    ...
    var photoViewMode = {
        "loop": function (index) {//loop
            ...
            return {
                ...
                next: index < t.size ? '#p=' + (index + 1) : '#p=1'
            };
        },
    ...
    ifeng.Gallery = function (options) {
        ...
        var o = this.o = {  //for easy using
            ...
            'btnOrig': gf('btnOrig'), 'showallPic': gf('showallPic'), 'photoIndex': gf('photoIndex'), 'photoCount': gf('photoCount'),
            'picDiv': gf('picDiv'), 'photoView': gf('photoView'), 'photoPrev': gf('photoPrev'), 'photoNext': gf('photoNext'), 'photo': gf('photo'), 'photoPrevLoading': gf('photoPrevLoading'), 'picList_b': gf('picList_b'),
            'photoLoading': gf('photoLoading'), 'photoDesc': gf('photoDesc'), 'kwords': gf('kwords'), 'photoDesc': gf('photoDesc'),
            'prevSet': gf('prevSet'), 'nextSet': gf('nextSet'), 'btnpic': gf('btnpic'), 'share_sq': gf('share_sq'),
            'boxScrl': gf('scrl'), 'thumb': gf('thumb'), 'bar': gf('bar'), 'btnScrlPrev': gf('scrlPrev'), 'btnScrlNext': gf('scrlNext'), 'scrlPrev_b': gf('scrlPrev_b'), 'scrlNext_b': gf('scrlNext_b'),
            'moretab': gf('moretab'), 'moreLeft': gf('moreLeft'), 'moreRight': gf('moreRight'), 'imageListView': gf('imageListView'), 'btnPage': gf('btnPage')
        }
        ...
        document.getElementById('photoCount').innerHTML = t.data.length;//设置图片总数量
        ...
        (function () {
            ...
            var _pone = document.createElement("h2");
            _pone.innerHTML = "1";
            var _spanone = document.createElement("span");
            _spanone.innerHTML = "/" + t.data.length;
            _pone.appendChild(_spanone);
            list[0].appendChild(_pone)
            for (var i = 0; i < len; i++) {
        ...
        O.setStyle(o.thumb, "width", (tWidth + 35 - fixpaddingRight - fixpaddingLeft) + 'px'); //ie6兼容增加2px
        ...
        (function () {
            ...
            while ((obj = t.loadlazyQuene.shift()) != null) {
                ...
                p.innerHTML = (t.size - t.loadlazyQuene.length);
                var _span = document.createElement("span")
                _span.innerHTML = "/" + t.size;
                li.appendChild(a);
                p.appendChild(_span)
                li.appendChild(p)
                o.thumbs.push(li);
        ...
        for (var i = 0; i < t.pageSize; i++) {
            ...
            addListener(a, 'click', function (e, val) {
                ...
                if (t.currentPageIndex != id) {
                    ...
                    gf(o.btnPage.id + "_" + t.currentPageIndex).className = "";
        ...
        o.showallPic && addListener(o.showallPic, "click", t.changeListMode.bind(t))
    ...
    ifeng.Gallery.prototype = {
        ...
        showPhoto: function (id) {
            ...
            if (info.timg != t.o.photo.src || index == 1) {
                ...
                gf('photoimg').innerHTML = "<img src='" + info.timg + "' id='photo' style='filter:alpha(opacity=100);'/>";
        ...
        resize: function (gfimg, size) {
            gfimg.removeAttribute('width');
            gfimg.removeAttribute('height');
            size = size || {};

            var rw = size.width ? gfimg.width / size.width : 0,
			rh = size.height ? gfimg.height / size.height : 0;

            if (rw > 1 || rh > 1) {
                rw > rh ? gfimg.width = size.width : gfimg.height = size.height;
            }
        },
    ...
    ifeng.Scroll.prototype = {
        ...
        _setPos: function (pos) {
            ...
            if (t._handle) {
                ...
                if (t._hPos == 0) {
                    gf("scrlPrev_b").className = "no";
                } else {
                    gf("scrlPrev_b").className = "";
                }
                gf("scrlNext_b").className = "";
                if (Math.ceil(t._hPos) >= t._hRange) {
                    gf("scrlNext_b").className = "no";
                    gf("scrlPrev_b").className = "";
                }
                if (t._hRange == 0) {
                    gf("scrlNext_b").className = "no";
                    gf("scrlPrev_b").className = "no";
                    gf("scrlPrev_b").style.cursor = "default";
                    gf("scrlNext_b").style.cursor = "default";
                }
