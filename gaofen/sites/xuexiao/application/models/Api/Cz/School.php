<?php
use ORM\CZ\School as ORM;

class Api_Cz_SchoolModel extends Api_BaseModel {
	...
	public function updatePreViewAndRangeRank($cate, $city='') {
		...
		//更新周排名(旧，不现实)
		/*
		$obj = School::select('id', 'range_views');
		//$obj->where('has_cz', '=', 1);
		...
		foreach($list as $num=>$row){
			School::where('id', $row['id'])->update(array($range_rank_field=>$num+1));
		}
		*/
		//更新周排名(新)
		$wheres = array();
