...
class Mailer extends phpMailer {
	...
	function smtp_mailer (){
  	 ...
  	 * @param  [type] $toList      接收到邮件列表； ex ['a@a.com', 'b@a.com', 'c@a.com']
  	 * @param  string $subject     邮件 主题
  	 * @param  string $body        邮件 内容
  	 * @param  array  $attachments 邮件附件；ex [['path'=>'/data/a.rar', 'name'=>'附件1'], ['path'=>'/data/b.rar', 'name'=>'附件2']]
  	 * @return [type]              [description]
  	 */
	function sendEmails($toList, $subject='s', $body='', $attachments=[]){
		//选项，输出编码
		$options  = array('out_charset' => 'UTF-8', 'format' => 1 );
		$from = array(GAOFEN_SEND_MAIL_FROM, '高分网');
		...
		if(is_array($attachments)) {
			foreach($attachments as $att){
				if(isset($att['path']) && isset($att['name'])) {
					$this->AddAttachment($att['path'], $att['name']);
				}
			}
		}
		
		$this->SMTPAuth = true;
		$this->setSMTP(GAOFEN_SMTP_HOST, GAOFEN_SMTP_USER, GAOFEN_SMTP_PASS, GAOFEN_SMTP_PORT);
