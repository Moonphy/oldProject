...
use Api\Base\Gz\School as SchoolApiBase;

class School extends ApiBase\Base
{
    private $_base = null;

    public function __construct(SchoolApiBase $schoolApiBase)
    {
     ...
     * @return array
     */
    public function getSchools(\Api\Apis\Gz\Search $search, array $params)
    {
        $params['page'] = OpenApi::param($params, 'page', 1);
        $params['count'] = OpenApi::param($params, 'limit', 10);
        $params['city'] = OpenApi::param($params, 'city', 289);

        $admitRange = new Support\AdmitRange($search);
        $school_ids = $admitRange->getSchools($params);

        $list = [];

        if ($school_ids['ids']) {
            $list = $this->_base->getBatch($school_ids['ids'], false, true);
        }
        return ['list' => $list, 'total' => $school_ids['total_number']];
    }
        ...
        foreach ($school_types as $school_type) {
            ...
            if (!empty($list1['ids'])) {
                ...
                foreach ($list1['ids'] as $k => $item) {
                    ...
                    $result[$school_type][$k]['url'] = $url .= \F::url('/gz/school/view',
