...
class Prize extends \Api\Apis\Base
{
    private $_base = null;

    public function __construct(\Api\Base\Huodong\Bestvoice\Prize $base)
    {
        $this->_base = $base;
    }
    ...
    public function search(array $p = array()) {
        ...
        $phone = OpenApi::param($p, 'phone');
        $limit = (int)OpenApi::param($p, 'limit', 20);
        $sql = sprintf('SELECT p.*,u.nickname,u.phone,u.extData FROM  `bestvoice_prize` AS p LEFT JOIN `bestvoice_user` AS u ON p.uid=u.id WHERE p.`type`="%d" AND u.`phone`="%s" LIMIT %d;',  $type, htmlentities($phone), $limit);
