...
class User extends \Api\Apis\Base
{
    private $_base = null;

    public function __construct(\Api\Base\Huodong\Bestvoice\User $base)
    {
        $this->_base = $base;
    }
    ...
    public function listByCond(array $p = array())
    {
        ...
        $sort = OpenApi::param($p, 'sort', 'updated_at');
        ...
        $ids = $this->_base->listIdsByCond($p, $sort);
     ...
     */
    public function getRankByUid(array $p= array()) {
        $uid = OpenApi::param($p, 'uid');

        return $this->_base->getRankByUid($uid);
    }
