...
class Assemblage extends \Api\Apis\Base
{
    private $_base = null;

    public function __construct(\Api\Base\Huodong\Bestvoice\Assemblage $base)
    {
        $this->_base = $base;
    }
    ...
    public function get($p = array())
    {
        ...
        //浏览数+1
        if ($info && $autoIncView) {
            if(is_numeric($autoIncView) && $autoIncView<0) {
                $this->_base->decrUnit('views', $id);
            } else {
                $this->_base->incrUnit('views', $id, $autoIncView, $views);
            }            
    ...
    public function getTotal(array $p = array()) {
        $type = OpenApi::param($p, 'type');

        $sql = sprintf('SELECT count(id) as cnt FROM `bestvoice_fav` WHERE `type`=%d',  $type);
        $rs = $this->_base->query($sql, 'select');

        return (int)@$rs[0]['cnt'];
    }

    public function setViews(array $p = array()) {
        $id     = OpenApi::param($p, 'id');
        $views  = OpenApi::param($p, 'views');

        return $this->_base->setUnit('views', $id, $views);
    }
...
///////==============================后台API=================================>
     ...
     * @return [type] [description]
     */
    private function getUcode() {
        $cookieName = 'EV_UCODE';
        $ucode = \F::escape(\F::getCookie($cookieName));
        if(!$ucode) {
            $ucode = uniqid();
            \F::setCookie($cookieName, $ucode, time() + 12*30*24*60*60); //过期时间一年
        }
