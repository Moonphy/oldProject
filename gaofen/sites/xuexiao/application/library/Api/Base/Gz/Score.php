...
class Score extends ApiBase\Base {

	static $_LIST_BY_TYPE_CACHE_KEY= 'gzscore:cond:list';
	static $_GET_CACHE_KEY 		= 'gzscore:id';
	static $_TABLE_UNIT_KEY		= 'gzscore:unit:id'; //浏览数缓存

	private $_model = NULL;

	function __construct(\Api_Gz_ScoreModel $model)
	{
		$this->_model = $model;
	}
	...
	*/
	public function save($data, $id='', $autoRefresh=true) {
		...
		$fields = array('sid'=>'number', 'area_id'=>'number', 'joins'=>'number', 'yiben_luqu'=>'float', 'erben_a_luqu'=>'float',
			'erben_b_luqu'=>"float", 'year'=>'number'
		);
	...
	*/
	...
	public function _getTableCondition() {
		...
		* 		val	表示单值条件
		*/
		return array('condition'=>array('sid'=>'val', 'year'=>'val'));
	}
