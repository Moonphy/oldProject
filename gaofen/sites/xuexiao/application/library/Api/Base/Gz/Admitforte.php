...
class Admitforte extends ApiBase\Base {

	static $_LIST_BY_TYPE_CACHE_KEY= 'gzAdmitforte:cond:list';
	static $_GET_CACHE_KEY 		= 'gzAdmitforte:id';
	static $_TABLE_UNIT_KEY		= 'gzAdmitforte:unit:id'; //浏览数缓存

	private $_model = NULL;

	function __construct(\Api_Gz_AdmitforteModel $model)
	{
		$this->_model = $model;
	}
	...
	*/
	public function save($data, $id='', $autoRefresh=true) {
		...
		$fields = array('sid'=>'number', 'year'=>'number', 'type'=>'number', 'cate'=>'number',
			'num'=>'number', 'memo'=>'string'
		);
    ...
    */
	...
	public function _getTableCondition() {
		...
		* 		val	表示单值条件
		*/
		return array('condition'=>array('sid'=>'val', 'year'=>'val', 'cate'=>'val', 'type'=>'val'));
	}
