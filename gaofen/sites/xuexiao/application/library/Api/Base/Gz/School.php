...
class School extends ApiBase\Base {
	...
	static $_LIST_BY_TYPE_CACHE_KEY = 'gzschool:cond:list';
	static $_GET_CACHE_KEY = 'gzschool:id';
	static $_TABLE_UNIT_KEY = 'gzschool:unit:id';//浏览数缓存

	private $_model = NULL;

	function __construct(\Api_Gz_SchoolModel $model) {
		$this->_model = $model;
	}
	...
	public function save($data, $id = '', $autoRefresh = true) {
		...
		$fields = array('name' => 'string', 'top' => '', 'gz_property' => 'int', 'province' => 'number', 'province_name' => 'string', 'city' => 'number',
			'city_name' => 'string', 'district' => 'number', 'district_name' => 'string', 'tags' => 'string',
			'ext' => '', 'sort' => 'number', 'views' => 'number', 'gz_state' => 'number', 'cover' => '', 'intro' => '',
			'has_gz' => '', 'has_cz' => '', 'school_zone' => 'string', 'sub_name' => 'string', 'cate' => 'number', 
			'gz_level'=>'int', 'gz_admit_range'=>'', 'gz_admit_batch'=>"", 'code'=>'number'
		);
	...
	public function listIdsByCond_db($cond, $page = NULL, $limit = NULL) {
		...
		}elseif(!empty($cond['has_gz'])) {
			$where[] = array('has_gz', '=', '1');
		}

		if ($cond['gz_property']!=='' && $cond['gz_property']!==NULL) {
			$where[] = array('gz_property', '=', (int)$cond['gz_property']);
		}
		...
		if (isset($cond['gz_state']) && in_array($cond['gz_state'], array('0', '1'))) {
			$where[] = array('gz_state', '=', $cond['gz_state']);
		}

		if (!empty($cond['gz_admit_range'])) {
			$where[] = array('gz_admit_range', 'andWhereInSet', array(1,$cond['gz_admit_range']));
		}
	...
	public function _getTableCondition() {
		 ...
		 * 		val	表示单值条件
		 */
		return array('condition' => array('topic' => 'val', 'gz_state' => 'val', 'has_gz' => 'val', 'cate' => 'val', 'city' => 'val'));
	}
	...
	private function _rowDataFormat($data, $useFormat = 1) {
		...
		$_formatFunc = function ($row) {
			...
			$row['link'] = \F::getSiteHost('xuexiao').\F::URL('/Gz/School/view', array('id'=>$row['id']));
			return $row;
		};
		...
		$_expandJsonFunc = function ($row) {
			...
			$row['gz_admit_range'] = !empty($row['gz_admit_range']) ? explode(',', $row['gz_admit_range']) : array();
			$row['gz_admit_batch'] = !empty($row['gz_admit_batch']) ? explode(',', $row['gz_admit_batch']) : array();
			$row['top'] = !empty($row['top']) ? explode(',', $row['top']) : array();
