...
class Info extends ApiBase\Base {

	static $_LIST_BY_TYPE_CACHE_KEY= 'gzinfo:cond:list';
	static $_GET_CACHE_KEY 		= 'gzinfo:id';
	static $_TABLE_UNIT_KEY		= 'gzinfo:unit:id'; //浏览数缓存

	private $_model = NULL;

	function __construct(\Api_Gz_InfoModel $model)
	{
		$this->_model = $model;
	}
	...
	*/
	public function save($data, $id='', $autoRefresh=true) {
		...
		$fields = array('sid'=>'number', 'type'=>'number', 'name'=>'string', 'sort'=>'number', 'data'=>'');
	...
	*/
	...
	public function _getTableCondition() {
		...
		* 		val	表示单值条件
		*/
		return array('condition'=>array('sid'=>'val', 'type'=>'val'));
	}
	...
	private function _rowDataFormat($data, $useFormat=1){
		...
		$_formatFunc = function($row) {
			if(is_array($row['data'])) {
				foreach($row['data'] as $_k=>$_v) {
					switch($_v['flag']) {
						case 'gz_admit_score':
							$row['data'][$_k]['value'] = array_values($this->getBase('/Gz/Admitscore')->getBatch(explode(',', $_v['value'])));
						break;

						case 'admit_forte' :
							//
							$ids = explode(',', $_v['value']);
							//目前需求只需要当年，为减少计算量，只取100条，足已！
							$ids = array_slice($ids, 0, 100);
							$row['data'][$_k]['value'] = array_values($this->getBase('/Gz/Admitforte')->getBatch($ids));
						break;

						case 'gz_score':
							$row['data'][$_k]['value'] = array_values($this->getBase('/Gz/Score')->getBatch(explode(',', $_v['value'])));
						break;

						case 'schedule':
							$row['data'][$_k]['value'] = array_shift($this->getBase('/Gz/Schedule')->get($_v['value']));
						break;
