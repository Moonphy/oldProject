...
class Weixin extends ApiBase\Base {

	static $_LIST_BY_TYPE_CACHE_KEY = 'auth:wx:cond:list';
	static $_GET_CACHE_KEY = 'auth:wx:id';
	static $_TABLE_UNIT_KEY = 'auth:wx:unit:id';//浏览数缓存

	private $_model = NULL;

	function __construct(\Api_Auth_WeixinModel $model) {
		$this->_model = $model;
	}
	...
	public function save($data, $id = '', $autoRefresh = true) {
		...
		$fields = array('uid'=>'int', 'unionid'=>'string', 'openid'=>'string', 'nickname'=>'string',
						'sex'=>'', 'city'=>'string', 'province'=>'string', 'headimgurl'=>'string',
						'access_token' => 'string', 'refresh_token'=>'string', 'expires_in'=>''
						);
	...
	public function listIdsByCond_db($cond, $page = NULL, $limit = NULL) {
		...
		if (!empty($cond['unionid'])) {
			$where[] = array('unionid', '=', "{$cond['unionid']}");
		}
		...
		foreach ($result as $row) {
			$ids[] = array('zScore' => strtotime($row['created_at']), 'zMember' => $row['uid']);
		}
	...
	public function getBatch_db($ids) {
		...
		return $this->formatKeyRows($rst, 'uid');
	}
	...
	public function _getTableCondition() {
		 ...
		 * 		val	表示单值条件
		 */
		return array('condition' => array('unionid' => 'val'));
	}
