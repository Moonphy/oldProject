...
class User extends ApiBase\Base {
	...
	static $_LIST_BY_TYPE_CACHE_KEY = 'auth:user:cond:list';
	static $_GET_CACHE_KEY = 'auth:user:id';
	static $_TABLE_UNIT_KEY = 'auth:user:unit:id';//浏览数缓存

	private $_model = NULL;

	function __construct(\Api_Auth_UserModel $model) {
		$this->_model = $model;
	}
	...
	public function save($data, $id = '', $autoRefresh = true) {
		...
		$fields = array('id' => 'int', 'name' => 'string', 'nickname' => 'string', 'tel'=>'', 'isTelValid'=>'');
