...
class Info extends ApiBase\Base {
	...
	static $_LIST_BY_TYPE_CACHE_KEY= 'czinfo:cond:list';
	static $_GET_CACHE_KEY 		= 'czinfo:id';
	static $_TABLE_UNIT_KEY		= 'czinfo:unit:id'; //浏览数缓存

	private $_model = NULL;

	function __construct(\Api_Cz_InfoModel $model)
	{
		$this->_model = $model;
	}
		...
		$_formatFunc = function($row) {
			if(is_array($row['data'])) {
				foreach($row['data'] as $_k=>$_v) {
					switch($_v['flag']) {
						case 'admit_score':
							$row['data'][$_k]['value'] = array_values($this->getBase('/Cz/Admitscore')->getBatch(explode(',', $_v['value'])));
						break;
						...
						case 'admit_forte' :
							...
							$row['data'][$_k]['value'] = array_values($this->getBase('/Cz/Admitforte')->getBatch($ids));
						break;

						case 'score':
							$row['data'][$_k]['value'] = array_values($this->getBase('/Cz/Score')->getBatch(explode(',', $_v['value'])));
						break;

						case 'schedule':
							$row['data'][$_k]['value'] = array_shift($this->getBase('/Cz/Schedule')->get($_v['value']));
						break;
