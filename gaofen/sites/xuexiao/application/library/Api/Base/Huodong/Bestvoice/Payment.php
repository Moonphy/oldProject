...
class Payment extends ApiBase\Base {

	use TableUnit;

	static $_LIST_BY_TYPE_CACHE_KEY = 'event:payment:list';
	static $_GET_CACHE_KEY = 'event:payment:id';
	static $_TABLE_UNIT_KEY = 'event:payment:unit:id';//浏览数缓存

	static $use_all_list = false;

	private $_model = NULL;

	function __construct(\Api_Huodong_Bestvoice_PaymentModel $model) {
		$this->_model = $model;
	}
	...
	public function save($data, $id = '', $autoRefresh = true) {
		...
		$fields = array('uid'=>'int', 'transaction_id'=>'', 'ip'=>'', 'type'=>'', 'subtype'=>'', 'meta'=>'');
	...
	public function listIdsByCond_db($cond, $sortByField='created_at', $page = NULL, $limit = NULL) {
		...
		if (!empty($cond['subtype'])) {
			$where[] = array('subtype', '=', $cond['subtype']);
		}
	...
	public function _getTableCondition() {
		 ...
		 * 		val	表示单值条件
		 */
		return array('condition' => array('uid' => 'val', 'subtype'=>'val'),
			'union' => ['type'=>'val'],
			);
	}
