...
class Prize extends ApiBase\Base {

	use TableUnit;

	static $_LIST_BY_TYPE_CACHE_KEY = 'event:prize:list';
	static $_GET_CACHE_KEY = 'event:prize:id';
	static $_TABLE_UNIT_KEY = 'event:prize:unit:id';//浏览数缓存

	static $use_all_list = false;

	private $_model = NULL;

	function __construct(\Api_Huodong_Bestvoice_PrizeModel $model) {
		$this->_model = $model;
	}
	...
	public function save($data, $id = '', $autoRefresh = true) {
		...
		$fields = array('id'=>'', 'pid'=>'int', 'uid'=>'int', 'ip'=>'string', 'type'=>'int',
				'gift'=>'string', 'fav_id'=>'int', 'date'=>'', 'created_at'=>'');
	...
	public function listIdsByCond_db($cond, $sortByField='created_at', $page = NULL, $limit = NULL) {
		...
		if (!empty($cond['gift'])) {
			$where[] = array('gift', '=', $cond['gift']);
		}

		if (!empty($cond['date'])) {
			$where[] = array('date', '=', $cond['date']);
		}
	...
	public function _getTableCondition() {
		 ...
		 * 		val	表示单值条件
		 */
		return array('condition' => array('uid'=>'val', 'pid'=>'val', 'gift'=>'val', 'date'=>'val'),
			'union' => ['type'=>'val'],
			);
	}
