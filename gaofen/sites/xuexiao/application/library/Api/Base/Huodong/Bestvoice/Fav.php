...
class Fav extends ApiBase\Base {

	use TableUnit;

	static $_LIST_BY_TYPE_CACHE_KEY = 'event:bvfav:cond:list';
	static $_GET_CACHE_KEY = 'event:bvfav:id';
	static $_TABLE_UNIT_KEY = 'event:bvfav:unit:id';//浏览数缓存

	static $use_all_list = false;

	private $_model = NULL;

	function __construct(\Api_Huodong_Bestvoice_FavModel $model) {
		$this->_model = $model;
	}
	...
	public function save($data, $id = '', $autoRefresh = true) {
		...
		$fields = array('id'=>'', 'mid'=>'int', 'uid'=>'int', 'ip'=>'string', 'ucode'=>'', 'type'=>'int', 'inviter'=>'');
	...
	public function _getTableCondition() {
		 ...
		 * 		val	表示单值条件
		 */
		return array('condition' => array('mid' => 'val', 'uid'=>'val'),
			'union' => ['type'=>'val'],
			);
	}
