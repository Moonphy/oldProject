...
class Assemblage extends ApiBase\Base {

	use TableUnit;

	static $_LIST_BY_TYPE_CACHE_KEY = 'event:assemblage:list';
	static $_GET_CACHE_KEY = 'event:assemblage:id';
	static $_TABLE_UNIT_KEY = 'event:assemblage:unit:id';//浏览数缓存

	static $use_all_list = false;

	private $_model = NULL;

	function __construct(\Api_Huodong_Bestvoice_AssemblageModel $model) {
		$this->_model = $model;
	}
	...
	public function save($data, $id = '', $autoRefresh = true) {
		...
		$fields = array('id'=>'', 'pid'=>'int', 'uid'=>'int', 'ip'=>'string', 'ucode'=>'', 'type'=>'int',
				'cond1'=>'int', 'cond2'=>'int', 'cond3'=>'', 'assemblage'=>'', 'views'=>'int'
			);
	...
	public function _getTableCondition() {
		 ...
		 * 		val	表示单值条件
		 */
		return array('condition' => array('pid' => 'val', 'uid'=>'val', 'type'=>'val', 'cond1'=>'val', 'cond2'=>'val', 'cond3'=>'val'),
			//'union' => ['uid'=>'val'],
			);
	}
	...
	private function _rowDataFormat($data, $useFormat = 1) {
		...
		$_formatFunc = function ($row) {
			$row->user = $row->uid?$this->getBase('/Huodong/Bestvoice/User')->get($row->uid):[];
			return $row;
		};

		//展开json数据
		$_expandJsonFunc = function ($row) {
			$row->assemblage = $row->assemblage ? json_decode($row->assemblage, true):[];
			return $row;
		};
