...
class  BaseCommonUtil {
	...
	function formatBizQueryParaMap($paraMap, $urlencode)
	{
		...
		foreach ($paraMap as $k => $v)
		{
			...
			{
				$v = str_replace('+', ' ', $v); //20150528增加，解决签证内容含空格时无法通过微信的签证验证
			}
			//$buff .= strtolower($k) . "=" . $v . "&";
			$buff .= $k . "=" . $v . "&";
		}
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$this->getCfg('mch_key');
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, $this->getCfg('sslcert'));
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $this->getCfg('sslkey'));
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
	...
	public function getCfg($field){
		...
		if(empty($cfgObj)){
			$catid = $this->getCatId();
			if(empty($catid)){
				throw new SDKRuntimeException('必须指定catid');
			}

			$cfgObj = new WxPayConf_pub($catid);
		}
