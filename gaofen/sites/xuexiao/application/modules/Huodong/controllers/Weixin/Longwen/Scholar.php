...
class Weixin_Longwen_ScholarController extends Yaf_Controller_Abstract
{	

    public function init() {
        $this->userObj = new User();
        $this->type = CFG::huodong('longwen', 'type');
        ...
        $this->favTTL = 60*60; //投票间隔
        $this->getView()->assign('favTTL', $this->favTTL);
    }
        ...
        if(!$this->userObj->isLogin()) {
            if(!$this->userObj->autoLogin()) {
                ...
                F::redirect(F::URL('auth:/Auth/Weixin/sync', 
                            array('catid'=>CFG::huodong('longwen', 'catid'), 'callback'=>F::URL('huodong_m:huodong/weixin_account/callback',['callback'=>$callback]))));
            }            
        }
    }

    public function indexAction() {
        F::redirect(F::URL('huodong_m:huodong/weixin_longwen_scholar/list'));
    }
    ...
    public function listAction() {
        ...
        $sectionhtml = $this->getView()->render('weixin/longwen/scholar/voicesection.html');
    ...
    public function moreSectionAction() {
        ...
        $this->getView()->assign('list', $list);
        $this->getView()->assign('uid', $uid);
        $html = $this->getView()->render('weixin/longwen/scholar/voicesection.html');
    ...
    public function searchAction() {
        ...
        foreach ($rs as $key => $value) {
            $rs[$key]['link'] = F::URL('huodong_m:/huodong/weixin_longwen_scholar/view', ['id'=>$value['id']]);
        }
    ...
    public function doFavAction() {
        ...
        if(time()>strtotime('2016-04-16')) {
            F::ajaxRst(false, '100002', '活动已经束');
        }

        $rs = F::api('/Huodong/bestvoice/Material/get', ['id'=>$id]);

        $favRs = false;
        //时间控制每天能投1次

        if( empty($rs) || ($rs && !$rs->present()->isFavByUser($uid, '', $this->favTTL))) {
    ...
    function getAJaxUri() {
        $uri = [
            'dofav'=>F::URL('huodong_m:/huodong/weixin_longwen_scholar/dofav'),
            'moresection'=>F::URL('huodong_m:/huodong/weixin_longwen_scholar/moresection'),
            'upload'=>F::URL('huodong_m:/huodong/weixin_longwen_scholar/upload'),
            'login'=>F::URL('huodong_m:/huodong/weixin_longwen_scholar/login', ['callback'=>BCommon::getCurrentUrl()]),
            'view'=>F::URL('huodong_m:/huodong/weixin_longwen_scholar/view'),
            'prize'=>F::URL('huodong_m:/huodong/weixin_longwen_scholar/getPrize'),
            'submitinfo' => F::URL('huodong_m:/huodong/weixin_longwen_scholar/submitinfo'),
            'search'=>F::URL('huodong_m:/huodong/weixin_longwen_scholar/search'),
        ];
    ...
    function getShareContent() {
        ...
        $shareData = [];
        $shareData['img'] = 'http://file.gaofen.com/event/longwen/fav-icon.png';
        switch($action) {
            default:
                $shareData['title'] = '龙文征集令-寻找学霸';
                $shareData['content'] = '龙文征集令-寻找学霸';
            break;
        }
