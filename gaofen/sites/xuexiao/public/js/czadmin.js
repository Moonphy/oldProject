...
(function($, win, G){
    ...
    var luri = location.href.toLowerCase(), router = [	
		'czadmin/school/list',     //学校列表页
		'czadmin/school/edit?', //学校简介
		'czadmin/school/editinfo?',//学校概览 学生生活 招生信息
		'czadmin/tip/index'],//经验分享
...
/*-----------------------------------------------学校概览------------------------------------------------*/
			...
			function getEditTitle(target){
				...
				return '';
				// return (target || currentList).find('.name').text();
			}
			...
			var insertTo = function(newYear){
				var target;
				md.find('tbody tr:visible').each(function(i){
					var td =  $(this).find('td:eq(0)');
					if(td.length){
						var year = td.text().match(/\d+(\.\d+)?/g) || '';
			...
			md.find('.modal-body').on('click', '.form-horizontal .btn-sm', function(){// 添加
				if(flag === 'score' || flag === 'admit_score' || flag === 'admit_forte' || flag === 'schedule'){//升学成绩
					var formData = md.find('div.form-horizontal input[type="text"]'), result = checkYear(formData.eq(0).val());
					if((flag !== 'schedule'&& flag !== 'admit_forte') && result) Alert(result, 'error');//特长生年份可以相同
					else{
						...
						if(flag === 'score'){
							data = {
									sid : sid,
									score_avg : formData.eq(1).val(),
									attend_num : formData.eq(2).val(),
									city_score_avg : formData.eq(3).val(),
									t_h_ratio : formData.eq(4).val(),
									year : formData.eq(0).val()
								};
							url = G.getRouter('/School/updateScore');
						}else if(flag === 'admit_score'){//录取分数线
							url = G.getRouter('/School/updateAdmitScore');
							tempName = 'admit_scoreList';
							data = {
								sid : sid,
								score : formData.eq(1).val(),
								memo : formData.eq(2).val(),
								year : formData.eq(0).val()
							};	
				...
				if($(this).hasClass('btn-danger')){//删除
					...
					if(flag === 'score' || flag === 'admit_score' || flag === 'admit_forte' || flag === 'schedule'){
						tempTr = $(this).closest('tr').hide();						
						var td = tempTr.find('td');
						if(flag === 'score'){//升学成绩
							$(Util.parse(G.tpls.get('scoreListEdit'), {
								year : td.eq(0).text().match(/\d+(\.\d+)?/g),
								score : td.eq(1).text().match(/\d+(\.\d+)?/g)||'',
								num : td.eq(2).text().match(/\d+(\.\d+)?/g)||'',
								pscore : td.eq(3).text().match(/\d+(\.\d+)?/g)||'',
								hundreds : td.eq(4).text().match(/\d+(\.\d+)?/g)||''
							})).insertAfter(tempTr);
						}else if(flag === 'admit_score'){//录取分数线
								$(Util.parse(G.tpls.get('admit_scoreEdit'), {
								year : td.eq(0).text().match(/\d+(\.\d+)?/g),
								score : td.eq(1).text().match(/\d+(\.\d+)?/g)||'',
								memo : td.eq(2).text()||''
							})).insertAfter(tempTr);
					...
					if(flag === 'score' || flag === 'admit_score' || flag === 'admit_forte'){
						var editTr = $(this).closest('tr'), tds = editTr.find('input'), result = checkYear(tds.eq(0).val(), flag === 'admit_forte');		
						if(result ) Alert(result, 'error');
							...
							if(flag === 'score'){//升学成绩
								data = {
										...
										sid : sid,
										score_avg : tds.eq(1).val(),
										attend_num : tds.eq(2).val(),
										city_score_avg : tds.eq(3).val(),
										t_h_ratio : tds.eq(4).val(),
										year : tds.eq(0).val()
									};
								url = G.getRouter('/School/updateScore');
							}else if(flag === 'admit_score'){//录取分数线
			...
			md.on('click', '#surebtn', function(){// 确认
				if(listLock) return;
				var jqt = $(this);
				if(jqt.data('lock')) return;
				jqt.data('lock', 1);
				if(flag === 'score' || flag === 'admit_score' || flag === 'admit_forte'){//升学成绩\录取分数线\特长招生				
					...
					var text = tinymce.get('introEditor').getContent(), key = 'data['+currentList.index()+']',
						...
						updateInfo(params, function(r){
							...
							currentList.find('textarea').text(text);
							Alert('保存成功！');
							md.modal('hide');
							jqt.data('lock', '');
						})	
