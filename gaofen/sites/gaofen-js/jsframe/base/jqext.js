...
*/	   
	   ...
	   */
	    ...
	    multSlider : function(opt){
			...
			config.main.height(198);
			config.main.find('.contents').css('position' , 'absolute');
			var allPage = Math.ceil(this.itemCount/2), jqItems = this.find(this.itemTag), 
				itemWidth = jqItems.outerWidth(), tempItems = [], lock;
			config.main.css({'overflow':'visible'});
			function run(){
				var next;
				if(config.turnTo === 'left'){
					next = (config.currentIndex+1)%allPage;
					if(next < config.currentIndex){
						tempItems = [];
						for(var i=0;i<config.includeItem;i++){
							var moveToPx =  allPage * config.includeItem * itemWidth;
							jqItems.eq(i).css({'left' : moveToPx+'px', 'position' : 'relative'});
							tempItems.push(jqItems.eq(i));
						}
					}								
					config.main.animate({'marginLeft':itemWidth*-1*config.includeItem*(config.currentIndex+1)}, config.sliderTimer,  function(){
							if(tempItems.length){
								config.main.css({'marginLeft':'0'});
								$.each(tempItems, function(){
									$(this).css({'position':'static','left':'0'});										
								})
								tempItems = [];
							}							
					...
					next = config.currentIndex === 0 ? allPage-1 : (config.currentIndex -1 );	

					if(next > config.currentIndex){
						tempItems = [], startItem = next * config.includeItem;
						for(var i=0;i<config.includeItem;i++){
							var item = jqItems.eq(startItem+i);
							if(item.length){
								var moveToPx =  -1 * (config.includeItem -i)*itemWidth;
								item.css({'left' : moveToPx+'px', 'position' : 'absolute'});
								tempItems.push(item);
							}
						}
					}
					
					config.main.animate({'marginLeft':itemWidth*config.includeItem*next}, config.sliderTimer, function(){
						if(tempItems.length){
							config.main.css({'marginLeft':(allPage -1)*config.includeItem *itemWidth *-1});
							$.each(tempItems, function(){
								$(this).css({'position':'static','left':'0'});
							})
							tempItems = [];
				...
				if(config.pageArea && !config.pageArea.hasClass('hidden')){
					setBtn(next);
			...
			function setBtnArea(){
				if(allPage > 1){
					config.pageArea.removeClass('hidden');
				}
			}
			
			function setBtn(next){	
				if(next === 0){
					config.controlerLeft.removeClass('prev-disabled');
					config.controlerRight.addClass('next-disabled');
				}
				
				if(next === allPage-1){
					config.controlerLeft.addClass('prev-disabled');
					config.controlerRight.removeClass('next-disabled');
				}
			}
			
			function setPageNumber(page){
				config.pageNumber.html('<i>'+page+'</i>/<i>'+allPage+'</i>');
			}
			
			
			return {
				init : function(){	
					if(config.autoRun) this.setRun();
					setBtnArea();
					setBtn(config.currentIndex);
					var that = this;
					if(config.controlerLeft && config.controlerLeft){
						config.controlerLeft.on('click', function(e){
							that.torun(e, 'left');
						})
						config.controlerRight.on('click', function(e){
							that.torun(e, 'right');
						})
					}
				},
				...
				setRun : function(){							
					config.timer = setInterval(function(){run()}, config.timeout);
					//config.timer = setTimeout(function(){run()}, config.timeout);
				},
				
				torun : function(e, type){
					e.preventDefault();
					var that = this;
					if(!lock){
