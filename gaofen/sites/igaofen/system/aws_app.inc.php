...
class AWS_APP
{
	...
	public static function login()
	{
		if (! AWS_APP::user()->get_info('uid'))
		{
			...
			{
				if(load_class('core_uri')->controller=='main' && load_class('core_uri')->action=='download' && isset($_SERVER['HTTP_REFERER'])) {
					$returnUrl = $_SERVER['HTTP_REFERER'];
				} else {
					$returnUrl = $_SERVER['REQUEST_URI'];
				}
				
				HTTP::redirect('/account/login/url-' . base64_encode($returnUrl));
			}
		}
	}
