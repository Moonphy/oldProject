...
class notify_class extends AWS_MODEL
{
	...
	static $only_specialist = false; //false返回系统原有通知，true只返回家分专家的通知
	...
	public function mark_read_all($uid)
	{
		...
		if($this->is_only_specialist()) {
			$where[] = $this->specialist_where('equal');
		} else {
			$where[] = $this->specialist_where('no_equal');
		}
	...
	public function delete_notify($where)
	{
		...
		//高分专家通知不走自动删除流程；(高分专家需求，后台加上)
		$where .= ' AND '. $this->specialist_where('no_equal');
	...
	{
		$where[] = 'recipient_uid = ' . intval($recipient_uid) . ' AND read_flag = 0';
		...
		if ($notification = $this->fetch_all('notification', implode(' AND ', $where), 'notification_id DESC'))
		{
			$notification_ids = array();
	 ...
	 */
	public function set_only_specialist($result=false) {
		static::$only_specialist = (bool)$result;
		return $this;
	}
	 ...
	 */
	public function is_only_specialist() {
		return (bool)static::$only_specialist;
	}
	 ...
	 * @return [type] [description]
	 */
	public function specialist_where($flag='equal') {

		$where = '';

		switch($flag) {
			case 'equal':
				$where =' `model_type` = ' . self::CATEGORY_SPECIALIST;
			break;
