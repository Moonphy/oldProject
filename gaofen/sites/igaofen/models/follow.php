...
class follow_class extends AWS_MODEL
{
	 ...
	 * @param  $limit
	 */
	public function get_user_fans($friend_uid, $limit = 20, $offset=0)
	{
		if (!$user_fans = $this->fetch_all('user_follow', 'friend_uid = ' . intval($friend_uid), 'add_time DESC', $limit, $offset))
		{
			return false;
		}
	 ...
	 * @param  $limit
	 */
	public function get_user_friends($fans_uid, $limit = 20 ,$offset=0)
	{
		if (!$user_follow = $this->fetch_all('user_follow', 'fans_uid = ' . intval($fans_uid), 'add_time DESC', $limit, $offset))
		{
			return false;
		}
	...
	/**
	 * [get_specialist_friends 获取关注的专家]
	 * @param  [type]     $fans_uid [uid]
	 ...
	 * @date   2016-09-26
	 */
	public function get_specialist_friends($fans_uid, $limit = 20 ,$offset=0){

		$users_sql="SELECT ";
        $users_sql.="uid ";
        $users_sql.="FROM aws_user_follow ";

        $users_sql.="LEFT JOIN aws_users ";
        $users_sql.="ON aws_user_follow.friend_uid = aws_users.uid ";

        $users_sql.="WHERE fans_uid = $fans_uid AND group_id = 100";

        $uids = $this->query_all($users_sql, $limit, $offset);

        foreach ($uids as $value) {
        	$specialist_uids[]=$value['uid'];
        }

		return $this->model('account')->get_user_info_by_uids($specialist_uids, true);
	}
}