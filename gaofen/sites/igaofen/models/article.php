...
class article_class extends AWS_MODEL
{
	public function get_article_info_by_id($article_id)
	{
		...
		$res = $this->format($articles[$article_id]);
	 ...
	 * @date   2017-01-03
	 */
	public function select_article($field=null, $where=null, $order=null, $limit=null, $offset=null){
		$res=$this->fetch_field_all('article', $field, $where, $order, $limit, $offset);
	...
	public function format($row) {		
		$row['article_cover_url_big']		= $this->get_cover_full_path($row['article_cover_url'], 'big');
		$row['article_cover_url_small']	= $this->get_cover_full_path($row['article_cover_url'], 'small');
		$row['article_cover_url']			= $this->get_cover_full_path($row['article_cover_url']);
		return $row;
	}
	 ...
	 * 查询文章、所属栏目以及所属专题数据
	 */
	public function select_article_column_special($where='', $field='', $limit=2, $offset=0, $order_by=''){
		if($where!=''){
			$where=$where;
		}else{
			$where=1;
		...
		$sql.="FROM aws_article ";

		$sql.="LEFT JOIN aws_column ON aws_article.column_id=aws_column.id ";
		$sql.="LEFT JOIN aws_special ON aws_article.special_id=aws_special.id ";
	 ...
	 * 查询文章、所属年龄段数据
	 */
	public function select_article_age($where='', $field='', $limit=2, $offset=0, $order_by='aws_article.id DESC'){
		if($where!=''){
			$where=$where;
		}else{
			$where=1;
		...
		$sql.="LEFT JOIN aws_age ON aws_article.age_id=aws_age.id ";
	...
	public function count_article($where = ''){
		if ($where) {
			...
			$count=$this->count('article');
		}
		return $count;
	}

	/**
	 * [update_article_ids 批量更新]
	 ...
	 * @date   2016-12-29
	 */
	public function update_article_ids($data,$ids){
		if(!empty($ids)){
			$article_ids_str = implode(',', $ids);
			$affect_num=$this->update('article',$data,"id in ($article_ids_str)");
			return $affect_num;
		}
	}

	/**
	 * [update_article_id 通过id更新]
	 * @param  [type]     $data [description]
	 ...
	 * @date   2017-01-04
	 */
	public function update_article_id($data,$id){
		if($id){
			$affect_num=$this->update('article',$data,"id = $id");
			return $affect_num;
		}
	}

	public function update_article_where($data,$where){
		$affect_num=$this->update('article',$data,$where);
			return $affect_num;
	}
	 ...
	 * @date   2017-01-05
	 */
	public function update_num($where, $column, $type = 'plus' ,$num = 1){
		if ($type == 'plus') {
			$sql="UPDATE aws_article SET $column = $column + $num ";
		}else{
			$sql="UPDATE aws_article SET $column = $column - $num ";
		}
		$sql.="WHERE $where";
