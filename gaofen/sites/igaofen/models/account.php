...
class account_class extends AWS_MODEL
{
    ...
    public function get_users_list($where, $limit = 10, $attrib = false, $exclude_self = true, $orderby = 'uid DESC', $offset=0)
    {
        if ($where)
        {
        ...
        $result = $this->fetch_all('users', $where, $orderby, $limit, $offset);
    ...
    /**
     * [fetch_user_uids uids查询用户信息]
     * @param  [type]     $uids [数组或字符串]
     * @param  [type]     $field [自选字段]
     * @return [type]           [按uids顺序返回结果]
     * @author ken
     * @date   2016-09-19
     */
    public function fetch_user_uids($uids,$field='*'){
        if(is_array($uids)){
            $uids=implode(',', $uids);
        }
