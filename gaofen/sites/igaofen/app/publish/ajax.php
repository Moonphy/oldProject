...
class ajax extends AWS_CONTROLLER
{
    ...
    public function modify_question_action()
    {
        ...
        if (!empty($_FILES['question_cover']['name'])) {
            ...
            $attach_id = $this->model('publish')->add_attach_with_item_id($item_type, $question_id, $upload_data['orig_name'], $_GET['attach_access_key'], time(), basename($upload_data['full_path']), $upload_data['is_image']);
        ...
        $this->model('question')->update_question_id($data,$question_id);
    ...
    public function publish_question_action()
    {
        ...
        {
            // H::p($_FILES['question_cover']['name']);
            ...
            if (!empty($_FILES['question_cover']['name'])) {
                ...
                $attach_id = $this->model('publish')->add_attach_with_item_id($item_type, $item_id, $upload_data['orig_name'], $_GET['attach_access_key'], time(), basename($upload_data['full_path']), $upload_data['is_image']);
			...
			//新问题提醒功能  hc  2016-6-27
			file_get_contents("http://wei.gaofen.com/mail.php?id=".$question_id."&username=".$this->user_info['user_name']."&subject=".$_POST['question_content']); 
    ...
    public function publish_article_action()
    {
        ...
        if (!$_POST['column_id'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择栏目')));
        }
            ...
            )))
        {
            $this->model('publish')->publish_approval('article', array(
                ...
                'parent_column_id' => $_POST['parent_column_id'],
        ...
        else
        {
            ...
            if (!empty($_FILES['article_cover']['name'])) {
                $item_type = 'article';
                $files_name = $_FILES['article_cover']['name'];
                $cover_url = $this->upload_file_action($item_type, $item_id = '', $files_name);
            }else{
                $cover_url = '';
            }
            ...
            $this->model('article')->update_article_id($data,$article_id);
    ...
    public function upload_file_action($item_type = 'article', $item_id, $files_name){

        $cover_name = $item_type.'_cover';

        $cover_url = $item_type.'_cover_url';
        ...
        if(!empty($files_name)) {
            ...
            if(!empty($attach_info['attachment'])) {
                $cover_url   = str_replace(get_setting('upload_url'), '', $attach_info['attachment']);
            }
        }
    ...
    public function modify_article_action()
    {
        ...
        if (!empty($_FILES['article_cover']['name'])) {
            $item_type = 'article';
            $files_name = $_FILES['article_cover']['name'];
            $cover_url = $this->upload_file_action($item_type, $item_id = $article_id, $files_name);
            $data['article_cover_url'] = $cover_url;
        }
    ...
    // 获取二级栏目
    public function get_column_action()
    {
        if ($_REQUEST['parent_column_id']) {
            $parent_column_id = $_REQUEST['parent_column_id'];
            $field = ['id','parent_id','title'];
            $where = "parent_id = $parent_column_id";
            echo json_encode($this->model('column')->select_column($field, $where));
        }
    }
}
