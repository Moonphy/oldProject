...
class weixin extends AWS_CONTROLLER
{
	...
	public function redirect_action()
	{
		...
		if ($_GET['uid'] AND get_setting('weixin_app_id') AND get_setting('weixin_app_secret'))
		{
			if (!$_GET['redirect'])
			{
			...
			if ($access_token = $this->model('openid_weixin_weixin')->get_sns_access_token_by_authorization_code($_GET['uid']))
			{
				if ($access_token['errcode'])
				{
					H::redirect_msg('授权失败: Redirect ' . $access_token['errcode'] . ' ' . $access_token['errmsg'] . ', Code: ' . htmlspecialchars($_GET['uid']));
				}
				...
				if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_openid($access_token['openid']))
				{
					...
					$this->model('gaofen_account')->map_uid_to_gaofen_uid($weixin_user['uid'], $access_token['uid']);
			...
			{
				H::redirect_msg('远程服务器忙,请稍后再试, State: ' . htmlspecialchars($_GET['state']) . ', Code: ' . htmlspecialchars($_GET['uid']));
			}
		}
		else
		{
	...
	public function authorization_action()
	{
		...
		else if ($_GET['uid'] OR $_GET['state'] == 'OAUTH' OR $_GET['state'] == 'OAUTH_REDIRECT')
		{
			if ($_GET['state'] == 'OAUTH')
			{
			...
			else
			{
				...
				$access_token = $this->model('openid_weixin_weixin')->get_sns_access_token_by_authorization_code($code);
			}
			...
			if ($access_token)
			{
				if ($access_token['errcode'])
				{
					H::redirect_msg('授权失败: Authorization ' . $access_token['errcode'] . ' ' . $access_token['errmsg'] . ', Code: ' . htmlspecialchars($_GET['uid']));
				}
	...
	public function oauth_redirect_action()
	{
		...
		//
		HTTP::redirect($this->model('gaofen_weixin')->loginRedirect($redirect_uri, $_GET['state'], $_GET['scope']));
	...
	public function register_action()
	{
		...
		if ($_GET['uid'] AND get_setting('weixin_app_id'))
		{
			if (!$access_token = $this->model('openid_weixin_weixin')->get_sns_access_token_by_authorization_code($_GET['uid']))
			{
				H::redirect_msg('远程服务器忙,请稍后再试, Code: ' . htmlspecialchars($_GET['uid']));
			}

			if ($access_token['errcode'])
			{
				H::redirect_msg('授权失败: Register ' . $access_token['errcode'] . ' ' . $access_token['errmsg'] . ', Code: ' . htmlspecialchars($_GET['uid']));
			}
	...
	{
		if (!$this->user_id AND $_GET['uid'])
		{
			HTTP::redirect(get_js_url('/m/weixin/authorization/?redirect=' . urlencode(base64_encode(get_js_url('/m/weixin/qr_login/?token=' . $_GET['token']))) . '&code=' . $_GET['uid'] . '&state=' . $_GET['state']));
		}
