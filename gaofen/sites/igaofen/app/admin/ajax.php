...
class ajax extends AWS_ADMIN_CONTROLLER
{
    ...
    // 文章手动排序
    public function article_sort_action(){
       if (!$_REQUEST['article_id'])
        {  
            H::ajax_json_output(AWS_APP::RSM(null, 40001, AWS_APP::lang()->_t('非法操作')));
        }

        $article_id = $_REQUEST['article_id'];
        $sort = $_REQUEST['sort'];

        $affect = $this->model('article')->update_article_id(['sort'=>$sort], $article_id); 
    ...
    // 问题手动排序
    public function question_sort_action(){
       if (!$_REQUEST['question_id'])
        ...
        $affect = $this->model('question')->update_question_id(['sort'=>$sort], $question_id); 
    ...
    public function topic_tips_show_action()
    {
        $tips_id  = $_POST['tips_id'];
        $tips_type = $_POST['tips_type'];
        ...
        $affect = $this->model('specialist_topicTips')->update_tips($update_data, 'id='.$tips_id);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    public function topic_show_action(){
        ...
        $affect = $this->model('specialist_topic')->update_topic($update_data, 'topic_id='.$topic_id);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    public function special_show_action(){
        ...
        $affect = $this->model('specialist_special')->update_special($update_data, 'id='.$special_id);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    // 将方法从专辑中移除
    public function special_topic_del_action(){

        if (!$_REQUEST['special_id'])
        {  
            H::ajax_json_output(AWS_APP::RSM(null, 40001, AWS_APP::lang()->_t('非法操作')));
        }
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, 40002, AWS_APP::lang()->_t('非法操作')));
        }
        ...
        // 方法的special_id归零
        $this->model('specialist_topic')->update_topic(['special_id'=>0], "topic_id = $topic_id");

        // 专辑方法总数-1
        $this->model('specialist_special')->calc_by_id_by_field($special_id, 'topic_num', $value=1, $method='decr');

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    public function special_sort_action(){
        ...
        $affect = $this->model('specialist_special')->update_special(['sort'=>$sort],"id = $special_id"); 

        if($affect){
            H::ajax_json_output(AWS_APP::RSM(null, 1, null));
        }
        H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('修改失败')));
    }

    public function order_refund_action(){
        ...
        $affect = $this->model('specialist_order')->update_order(['refund_type'=>1],"order_id = $order_id");
        if($affect){
            H::ajax_json_output(AWS_APP::RSM(null, 1, null));
        }
    ...
    /**
     * [column_show_action 文章栏目是否显示]
     * @return [type]     [description]
     * @author ken
     * @date   2016-12-26
     */
    public function column_show_action(){
        ...
        if(!is_digits($_REQUEST['is_show']) && $_REQUEST['is_show'] >1){
            H::ajax_json_output(AWS_APP::RSM(null, 40002, AWS_APP::lang()->_t('非法操作')));
        }
        ...
        $affect = $this->model('column')->update_column($update_data, 'id='.$column_id);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    // 批量删除栏目
    public function column_manage_action()
    {
        if (!$_POST['column_ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择栏目进行操作')));
        }
        ...
        switch ($_POST['action'])
        {
            case 'del':
                $data = ['is_del' => 1];
                $affect = $this->model('column')->update_column_ids($data, $column_ids);

                foreach ($column_ids as $value) {
                    // 文章表中column_id和parent_column_id是当前栏目的全部清零
                    $this->model('article')->update_article_where(['column_id'=>0],"column_id = $value");
                    $this->model('article')->update_article_where(['parent_column_id'=>0],"parent_column_id = $value");
                }
    ...
    // 栏目排序
    public function column_sort_action(){
        ...
        $affect = $this->model('column')->update_column(['sort'=>$sort],"id = $column_id"); 
    ...
    // 批量管理栏目文章
    public function column_article_manage_action()
    {
        if (!$_POST['article_ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择文章')));
        }
        ...
        switch ($_POST['action'])
        {
            // 解除当前栏目下的文章
            case 'del':
                $data = ['column_id' => 0,'parent_column_id' => 0];
                $affect = $this->model('article')->update_article_ids($data, $article_ids);
            ...
            case 'push':
                if ($_POST['special_id']) {
                    ...
                    H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('前选择推送专题')));
                }
                ...
                $article_list = $this->model('article')->select_article($field, $where);
                foreach ($article_list as $value) {
                    $special_ids[] = $value['special_id'];
                } 
                // 需要更改的专题数，key为原专题id,value为次数
                $special_ids = array_count_values($special_ids);
                // 批量减去原专题的文章总数
                foreach ($special_ids as $key => $value) {
                    if($key != 0){
                        $this->model('special')->update_special_num("id = $key", 'article_num', $type = 'sub' ,$num = $value);
                    }
                }
                ...
                if($affect){
                    // 更新专题中的文章数量
                    // $this->model('special')->update_special(['article_num'=>$affect],"id = $special_id");
                    $this->model('special')->update_special_num("id = $special_id", 'article_num', $type = 'plus' ,$num = $affect);
            ...
            case 'add':
                ...
                // 查询父栏目id
                $parent_column_info = $this->model('column')->get_column($where = "id = $column_id",$field = ['parent_id']);
                // 插入父栏目id
                $data['parent_column_id'] = $parent_column_info['parent_id'];
                $affect = $this->model('article')->update_article_ids($data, $article_ids);
    ...
    // 批量删除栏目
    public function special_manage_action()
    {
        if (!$_POST['special_ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择栏目进行操作')));
        }

        $special_ids = $_POST['special_ids'];

        switch ($_POST['action'])
        {
            case 'del':
                $data = ['is_del' => 1];
                $affect = $this->model('special')->update_special_ids($data, $special_ids);
    ...
    // 专题排序
    public function article_special_sort_action(){
        ...
        $affect = $this->model('special')->update_special(['sort'=>$sort],"id = $special_id"); 
    ...
    // 专题是否推荐
    public function article_special_recommend_action(){
        ...
        $affect = $this->model('special')->update_special(['is_recommend'=>$is_recommend],"id = $special_id"); 
    ...
    // 批量删除专题文章
    public function special_article_manage_action()
    {
        if (!$_POST['article_ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择栏目进行操作')));
    ...
    /**
     * [article_special_show_action 前台是否显示专题]
     * @return [type]     [description]
     * @author ken
     * @date   2016-12-29
     */
    public function article_special_show_action(){
        ...
        if(!is_digits($_REQUEST['is_show']) && ($_REQUEST['is_show']>1 || $_REQUEST['is_show']<0)){
            H::ajax_json_output(AWS_APP::RSM(null, 40002, AWS_APP::lang()->_t('非法操作')));
        }
        ...
        $affect = $this->model('special')->update_special($update_data, 'id='.$special_id);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    // 批量管理年龄段
    public function age_manage_action()
    {
        if (!$_POST['age_ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择年龄段进行操作')));
        }

        $age_ids = $_POST['age_ids'];

        switch ($_POST['action'])
        {
            case 'del':
                $data = ['is_del' => 1];
                $affect = $this->model('age')->update_age_ids($data, $age_ids);

                foreach ($age_ids as $value) {
                    // 文章表中age_id是当前年龄段的全部清零
                    $this->model('article')->update_article_where(['age_id'=>0],"age_id = $value");
                }
    ...
    // 批量管理年龄段文章
    public function age_article_manage_action()
    {
        if (!$_POST['article_ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择文章')));
        ...
        switch ($_POST['action'])
        {
            ...
            case 'add':
                if ($_POST['age_id']) {
                    ...
                    H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('前选择年龄段')));
                }
    ...
    /**
     * [activity_show_action 活动是否显示]
     * @return [type]     [description]
     * @author ken
     * @date   2016-12-26
     */
    public function activity_show_action(){
        ...
        $affect = $this->model('activity')->update_activity($update_data, 'id='.$activity_id);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    // 主页轮播管理
    public function loop_index_action(){
        
        if (!$_REQUEST['loop_id'] && !$_REQUEST['name'])
        {  
            H::ajax_json_output(AWS_APP::RSM(null, 40001, AWS_APP::lang()->_t('非法操作')));
        }

        $loop_id = $_REQUEST['loop_id'];
        $name = $_REQUEST['name'];
        $type = $_REQUEST['type'];
        $val = $_REQUEST['val'];
        ...
        $affect = $this->model('loop')->update_loop($data,"id = $loop_id"); 
