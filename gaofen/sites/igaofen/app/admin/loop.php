...
class loop extends AWS_ADMIN_CONTROLLER
{
	...
	public function list_action()
	{
		if ($this->is_post())
		{
			...
			H::ajax_json_output(AWS_APP::RSM(array(
				'url' => get_js_url('/admin/loop/list/' . implode('__', $param))
			), 1, null));
		}


		$where = array("`type`='loop'");
		...
		TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
			'base_url' => get_js_url('/admin/loop/list/') . implode('__', $url_param),
			'total_rows' => $search_loops_total,
			'per_page' => $this->per_page
		))->create_links());

		$this->crumb(AWS_APP::lang()->_t('文章管理'), 'admin/loop/list/');

		TPL::assign('loops_count', $search_loops_total);
		TPL::assign('list', $loops_list);

		TPL::output('admin/loop/list');
	}


	public function edit_loop_action(){
		if($_GET['id']){
			$id = $_GET['id'];
		}else{
			HTTP::redirect('/admin/loop/list/');
		}

		$loop_info = $this->model('loop')->get_loop_by_id($id);
		$this->crumb(AWS_APP::lang()->_t('文章编辑'), 'admin/loop/edit/');

		TPL::assign('loop_info', $loop_info);
		TPL::output('admin/loop/edit');
	}


	public function update_loop_action(){
		$id = $_POST['id'];
